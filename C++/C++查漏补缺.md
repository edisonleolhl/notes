...
4. 类的静态成员，静态成员可以实现多个对象之间的数据共享，并且使用静态数据成员还不会破坏隐藏的原则，即保证了安全性。因此，静态成员是类的所有对象中共享的成员，而不是某个对象的成员，注意不能在类中初始化，也不能在main函数中初始化。
...
2. 虚函数是为了实现运行时多态，即指向父类的指针也可以指向子类，并且调用子类的虚函数

3. 每一个具有虚函数的类都有一个虚函数表，里面按在类中声明的虚函数的顺序存放着虚函数的地址，这个虚函数表是这个类的所有对象所共有的，也就是说无论用户声明了多少个类对象，但是这个虚函数表只有一个。

4. 在有虚函数的类中，类的对象的头部是一个指向虚函数表的指针，每个类的对象头部都有虚函数表指针

5. 子类继承父类时也会继承虚函数表，但是当子类重写继承的虚函数时，子类的虚函数表中的对应虚函数地址会替换为重写的虚函数地址。
...
- 虚继承一般通过vbptr虚基类指针和vb虚基类表实现，每个虚继承的子类都有一个虚基类指针（占用一个指针的存储空间，4字节，放在虚函数表指针的后面，如果没有虚函数表指针，那就放在类实例的头部）和虚基类表（不占用类对象的存储空间）；当虚继承的子类被当做父类继承时，虚基类指针也会被继承。
- 解决了二义性问题，解决了钻石继承/菱形继承/重复继承问题，也节省了内存，避免了数据不一致的问题。

### 含有虚基类的构造函数/析构函数的执行顺序

- 首先执行虚基类的构造函数，多个虚基类的构造函数按照被继承的顺序构造（若没有虚基类，则略过这条）；
...
- sizeof(b)=1：编译器为空类安插1字节的char，以使该类对象在内存得以配置一个地址
- sizeof(b1)=8：b1虚继承于b，编译器为其安插一个8字节的虚基类表指针（64位机器），此时b1已不为空，编译器不再为其安插1字节的char
- sizeof(b2)=8：b2同理
- sizeof(d)=16：d含有来自b1与b2两个父类的两个虚基类表指针，加起来大小为16字节

新增：类对象的大小=各非静态数据成员（包括父类的非静态数据成员但都不包括所有的成员函数）的总和+ vfptr指针(多继承下可能不止一个)+vbptr指针(多继承下可能不止一个)+编译器额外增加的字节（补齐padding），其中补齐操作跟编译器有关，暂时不用深究。
...
1. 管理方式：栈是用数据结构栈实现的，由系统分配，速度较快，但不受程序员控制，不够则报栈溢出的错误；堆是由链表实现的，由程序员手动配置（new和delete），速度较慢，也容易产生内存泄露，但使用灵活
...
用户态和内核态是操作系统的两种运行权限级别，内核态拥有的权限更高，运行在用户态的进程不能访问内核态的资源
...
1. 协程/微线程/纤程(Coroutine)是一种无优先级的子程序调度组件，允许子程序在特点的地方挂起恢复。
2. 线程包含于进程，协程包含于线程
3. 协程最大的优势就是协程极高的执行效率，由于子程序切换没有线程切换开销高，线程数量越多，协程的性能优势越明显
...
对文件进行写操作, c语言的open, write, fork, vfork等等都是系统调用
...
3. fork()的父子进程的执行次序不确定；vfork()保证子进程先运行，在调用exec或exit之前与父进程数据是共享的，在它调用exec或exit之后父进程才可能被调度运行，如果调用exec或exit之前子进程依赖于父进程的某些动作，则会导致死锁

### 为什么要有vfork()

在实现写时复制之前，为了避免fork后立刻执行exec所造成的地址空间的浪费
...
为了解决文件共享问题，Linux引入了软链接和硬链接。还带来了隐藏文件路径、增加权限安全及节省存储等好处。
...
陈硕编写的muduo是一个高质量的reactor网络库，代码简洁，但无法跨平台
...
优点：没有最大并发连接的限制；效率提升，不是轮询的方式，不会随着FD数目的增加效率下降；epoll使用mmap将文件映射进内存加速消息传递
...
libev 库与 libevent 库一样，都是基于事件循环的系统，它们在 poll()、select() 等机制的本机实现的基础上提供基于事件的循环

创建所需的网络监听套接字，注册在执行期间要调用的事件，然后启动主事件循环，让 libev 处理过程的其余部分

### 请你说说select、poll、epoll的区别

1. 支持一个进程打开的最大连接数：select由FD_SETSIZE宏定义，32位机器就是`32*32`，64位机器就是`32*64`；poll没有最大连接数的限制，因为基于链表存储；epoll虽然连接数有上线，但是很大
2. FD加剧带来的效率问题：select因为每次调用时都会对连接线性遍历，所以性能会下降；poll同理；epoll因为内核实现是根据每个fd上的callback函数实现的，只有活跃的socket才会主动调用callback，所以没有select/poll那样的性能下降问题
3. 消息传递方式：select内核需要将消息传递到用户空间，需要内核拷贝动作；poll同理；epoll通过内核和用户空间共享一块内存来实现
...
1. 允许将TIME-WAIT sockets重新用于新的TCP连接：/etc/sysctl.conf，加入`net.ipv4.tcp_tw_reuse = 1`
2. 开启TCP连接中TIME-WAIT sockets的快速回收：/etc/sysctl.conf，加入`net.ipv4.tcp_tw_recycle = 1`
3. 简单来说，就是打开系统的TIMEWAIT重用和快速回收。
...
在流传输中出现，UDP不会出现粘包，因为它有消息边界，TCP是基于字节流的传输，从缓冲区看，后一个包的包头和前一个包的包尾紧挨着，接收端分不清这几个包是粘在一起的，所以会造成粘包
...
2. 浏览器线解析URL得到域名，通过域名找IP地址，这需要域名解析系统DNS，DNS是基于UDP的，首先会查浏览器的DNS缓存，然后查本机的DNS缓存(hosts文件），再然后查本地域名服务器，这里的查询方式分为迭代查询和递归查询，迭代：本地域名服务器-根域名服务器-本地域名服务器-顶级域名服务器-权限域名服务器，递归：本地域名服务器-根域名服务器-顶级域名服务器-权限域名服务器
...
- Redis是一个开源的非关系型数据库，数据全部存在内存中，定期写入磁盘，当内存不够时，可以选择指定的LRU算法删除数据
- Redis支持多种数据结构，如hash、set、list等
...
- 使用场景：要求生成唯一序列号；WEB 中的计数器，不用每次刷新都在数据库里加一次，用单例先缓存起来；创建的对象消耗资源过多，比如 I/O 与数据库的连接等。
- 优点：减少了内存的开销；避免对资源的多重占用
- 缺点：没有接口，不能继承

### 单例模式的实现方法

- 懒汉式（延迟加载唯一实例）：第一次使用实例时才会加载
    1. 静态指针+用时初始化：线程不安全，内存泄漏（可以用内嵌类删除单例的唯一实例）
    2. 局部静态变量：线程不安全
- 饿汉式（直接加载唯一实例）：定义单例类时就进行实例化，没有多线程的问题
    1. 直接定义静态对象：实现简单，多线程安全
    2. 静态指针+类外初始化时new空间实现：多线程安全
