...
auto sp1 = make_shared<widget>(), sp2{ sp1 }; // 分配一次内存，异常安全
```

分配两次内存：

- 原始的 new 表达式分配对象，shared_ptr需要申请两次内存块，一次是widiget的内存，一块是引用计数的内存
- make_shared，widget的内存与引用计数的内存是一起申请的

异常不安全：

```c++
processWidget(std::shared_ptr<Widge>(new Widget), computePriority()); //潜在的资源泄露
```

编译器指令重排，可能生成这样的机器代码：

1. 执行“new Widget”。
2. 执行computePriority。// 如果异常，那么new widget内存泄漏
...
如果使用std::unique_ptr和std::make_unique来替换std::shared_ptr和std::make_shared，事实上，会用到同样的理由
...
enable_shared_from_this 的常见实现为：其内部保存着一个对 this 的弱引用（例如 std::weak_ptr )
...
- malloc在申请内存时，一般会通过brk或者mmap系统调用进行申请。其中当申请内存小于128K时，会使用系统调用**brk在堆区**中分配（满足条件才能释放）；而当申请内存大于128K时，会使用系统调用**mmap在内存映射区**分配（可以马上归还给系统，但有可能不会）。mmap有一种用法是映射磁盘文件到内存中（进程间通信），如果是匿名映射，就不指定磁盘文件，也就相当于开辟了一块内存。
...
### main函数之前和之后做了什么/程序一定从 main 函数开始吗
...
在ld链接过程中使用-e参数可以指定程序入口，所以不一定是 main 函数开始
...
- 链接链的就是目标文件，什么是目标文件？目标文件就是源代码编译后但未进行链接的那些中间文件，如Linux下的.o，与可执行文件格式结构相似，都是 Linux 的 ELF 文件
...
- 目标文件主要分为文件头、代码段（程序源代码编译后的机器指令）、数据段（已经初始化的全局变量和局部静态变量的.data 段，未初始化的全局变量和局部静态变量的.bss段）和其它。
- .bss 不占程序空间
...
- 可以使用`readelf -h test.o`查看文件头：ELF魔数、文件机器字节长度、数据存储方式、版本、运行平台、ABI版本、ELF重定位类型、硬件平台、硬件平台版本、入口地址、程序头入口和长度、段表的位置和长度和段的数量。
- 使用`objdump -h test.o`查看ELF文件中包含的关键的段
- 使用`readelf -S test.o`查看ELF文件中包含的所有的段
...
- 作用：去除二进制文件中的符号，可以减少体积，也可以防止你的二进制产物被逆向
- strip后，nm命令就不起作用了
...
- 每一个共享库都有一个SO-NAME，即共享库的文件名去掉次版本号和发布版本号，比如"libname.so.x.y.z"的SO_NAME就是"libname.so.x"
- 在Linux系统中，系统会为每个共享库在所在的目录中创建一个与SO-NAME同名并且指向实际共享库的软链接。例如"libc.so.1"会指向"libc.so.1.2.3"，当目录中有"libc.so.1.2.4"时，"libc.so.1"这个软链就会指向"libc.so.1.2.4"
...
- 简单总结：/lib是内核级的，/usr/lib是系统级的：/usr/local/lib是用户级的。
...
- Debug模式下可以使用assert，运行过程中有异常现象会及时crash，Release模式下模式下不会编译assert，所以可能会产生奇奇怪怪的错误

- 编译器在Debug模式下定义_DEBUG宏，Release模式下定义NDEBUG宏，预处理器就是根据对应宏来判断是否开启assert的。
...
### include头文件的顺序/头文件双引号""和尖括号<>的区别/如何避免同一个文件被include多次
...
- push_back()函数的执行过程：先调用构造函数构造一个临时对象，然后在容器尾部的空间位置上以临时对象为参数调用移动构造函数构造一个新的对象，最后调用临时对象的析构函数释放临时对象。
- emplace_back()函数的执行过程：直接在容器尾部的空间位置上调用构造函数构造一个新对象。
...
- 两者都是只调用移动构造函数，如果类没有移动构造函数，则会转为调用拷贝构造函数
...
- 信号：可用做**异常事件**的通信，用于通知接收进程某个事件已经发生。主要作为进程间以及同一进程不同线程之间的同步手段。
  - 用`kill -l`命令可以知道所有信号，`kill -9 xx`可以产生SIGKILL信号，杀死pid为xx的信号；Ctrl+C 产生 SIGINT 信号，表示终止该进程；`kill -15`是kill命令默认的信号，即SIGTERM，告知进程做一些善后的工作，要安全干净地退出
...
- 自旋锁加锁失败后，线程会忙等待，直到它拿到锁；
  ...
  - 使用自旋锁的时候，当发生多线程竞争锁的情况，加锁失败的线程会「忙等待」，直到它拿到锁。这里的「忙等待」可以用 while 循环等待实现，不过最好是使用 CPU 提供的 PAUSE 指令来实现「忙等待」（仅x86支持，其他都是用while循环实现），因为可以减少循环等待时的耗电量。
...
可能MySQL服务端开启了缓存，开启了Query Cache且命中完全相同的SQL语句，则直接将结果返回给客户端，最后执行完之后也会把结果缓存
...
- 必须满足ACID（原子性、一致性、隔离性和持久性）
    ...
    5. MySQL的存储引擎InnoDB使用重做日志保证一致性与持久性，回滚日志保证原子性，使用各种锁来保证隔离性。
...
- **可重复读(repeatable read，默认隔离级别)**：在同一事务中多次读取的数据是一致的。
...
- 垂直拆分：按照字段(或者列)进行拆分，其实就是把组成一行的多个列分开，放到不同的表中
  - 场景：
    - 垂直分表：不常用的字段、text、blob等大字段单独放一张表
    - 垂直分库：按照业务进行拆分
...
分库分表按照某种partitionKey进行拆分后，可能在另外一个维度的查询比较困难，比如电商场景如果商品按照消费者uid进行拆分，那卖家就不好查询自己卖出去的所有商品，所以有两种方法

1. nonosql：mysql通过binlog同步到elasticSearch，es上冗余一份，商家去es查
2. 冗余双写：拆分成买家库和卖家库
...
MySQL通信协议是基于TCP 传输协议的，三次握手、四次挥手都很耗时
...
- 跳表可以加快查询速率，查询任意数据的时间复杂度就是O(logn)，但是索引是需要空间，对于长度为n的单链表，需要再消耗O(n)的空间构造跳表，但是实际上索引只是一些指针而已，如果数据是很大的对象的话，用空间换时间还是很划算的，而且跳表的插入、删除操作的时间复杂度也是O(logn)，跳表可以用O(logn)找到待插入/删除的位置，然后用O(1)完成插入/删除
...
思考另一个场景：两个并发操作，一个是更新操作，另一个是查询操作，更新操作删除缓存后，查询操作没有命中缓存，先把老数据读出来后放到缓存中，然后更新操作更新了数据库。于是，在缓存中的数据还是老的数据，导致缓存中的数据是脏的。很显然，这种状况也不是我们想要的。

为了确保请求 A 在睡眠的时候，请求 B 能够在这这一段时间完成「从数据库读取数据，再把缺失的缓存写入缓存」的操作，然后请求 A 睡眠完，再删除缓存。
...
但是具体睡眠多久仍然是玄学，只是尽量保证一致性，极端场景还是有不一致

#### cache aside

查询：先查询缓存缓存，未命中，则查db，查到了再回写缓存

更新：先更新db，然后删除缓存中对应key

这种模式可以很大程度保证一致性，不过理论上仍然有个极端场景（概率极低）：先读缓存未命中，读db，回写缓存的请求晚于更新db并删除缓存的请求，实际上，数据库写操作比读操作慢很多，而且还要锁表，所以查询请求要在写操作前进入数据库操作，而又要晚于写操作更新缓存，所以概率非常低
...
- 做法：
  ...
  - 阿里巴巴开源的中间件Canal就是基于binlog实现的，它模拟mysql主从复制的协议，把自己伪装成一个mysql从节点，向 MySQL 主节点发送 dump 请求，MySQL 收到请求后，就会开始推送 Binlog 给 Canal，Canal 解析 Binlog 字节流之后，转换为便于读取的结构化数据，供下游程序订阅使用。
...
技术特点：瞬时流量徒增、读多写少、数据一致性要求高

热点问题：

1. 热点操作：如零点下单、零点刷新、零点加购，用户行为不好改变，但可以加上保护措施，如限流提示阻断
2. 热点数据：如抢购茅台，无论是什么数据库or分布式缓存也扛不住几十万上百万的请求对同一个key的读写，热点数据处理分三步：识别、隔离、优化

解决超卖，防止资损

架构原则：

1. 尽量将请求拦截在上游，DNS、CDN、web网关、web服务、中间件服务、数据库
2. 数据要尽量少
3. 请求要尽量少
4. 路径尽量短，比如强依赖的应用部署在一起，RPC变JVM内部的方法调用
5. 依赖尽可能少

前端数据优化，动静分离，url动态，前端页面拦截（验证码、拼图、答题）
...
1. 业务隔离、系统隔离、数据隔离
2. 限流、削峰（验证码、拼图、答题），异步化消息队列
3. 缓存热点数据
4. 限购：商品限购（地域、数量）、个人（同一手机号、同一收货地址）

减库存：

1. 下单就减库存：因为抢到就是赚到，一般用这个方案
2. 付款减库存
3. 预扣库存：买家下单后，库存为其保留一定时间，超时释放

防止超卖：

1. 库存不能小于0
2. 缓存中减库存，适合逻辑单一的场景，没有sku库存和总库存联动关系
3. 数据库减库存，适合比较复杂的，或者需要事务的（应用层做排队，缓解数据库压力，也能控制热点商品占据太多数据库连接