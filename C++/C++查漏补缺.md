...
虚函数是为了实现运行时多态，即指向父类的指针也可以指向子类，并且调用子类的虚函数

在有虚函数的类中，类的对象的头部是一个虚函数表的指针，虚函数表中放了虚函数的地址

多态，是指在继承层次中，父类的指针可以具有多种形态，当它指向某个子类对象时，通过它能够调用到子类的函数，而非父类的函数，子类继承父类时也会继承虚函数表，但是当子类重写虚函数数时，虚函数表中的地址会替换为重写的函数地址。
...
- 虚继承一般通过虚基类指针和虚基类表实现，每个虚继承的子类都有一个虚基类指针（占用一个指针的存储空间，4字节）和虚基类表（不占用类对象的存储空间）；当虚继承的子类被当做父类继承时，虚基类指针也会被继承。
...
转自：[图说C++对象模型：对象内存布局详解](https://www.cnblogs.com/QG-whz/p/4909359.html)
...
- sizeof(b1)=4：b1虚继承于b，编译器为其安插一个4字节的虚基类表指针（32为机器），此时b1已不为空，编译器不再为其安插1字节的char
- sizeof(b2)=4：b2同理
- sizeof(d)=8：d含有来自b1与b2两个父类的两个虚基类表指针，加起来大小为8字节
