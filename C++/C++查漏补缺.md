...
- sizeof(b1)=8：b1虚继承于b，编译器为其安插一个8字节的虚基类表指针（64位机器），此时b1已不为空，编译器不再为其安插1字节的char（有些编译器做不到这样的优��，字节对齐后也有可能类对象的大小为16字节）
- sizeof(b2)=8：b2同理
...
- `upper_bound(first, last, k)`找到第一个大于查找键k的��素(k < x)
...
**快速适配(quick fit)**，它���那些常用大小的空闲区**维护单独的链表**。例如，有一个 n 项的表，该表的第一项是指向大小为 4 KB 的空闲区链表表头指针，第二项是指向大小为 8 KB 的空闲区链表表头指针，第三项是指向大小为 12 KB 的空闲区链表表头指针，以此类推。比如 21 KB 这样的空闲区既可以放在 20 KB 的链表中，也可以放在一个专门存放大小比较特别的空闲区链表中。（感觉类似C++ STL的空间配置器）
...
争论：我在晚上查阅了很多资料（知乎、stackoverflow、大学课件、论文插图，很多地方说可以把四次握手的中间两次合二为��，即server端发送的是FIN+ACK，也有人通过抓包证明了这一结论。
...
- $PS4：主��示符变量$PS1和第二级提示符变量$PS2比较常见，而$PS4的值将被显示在“-x”选项输出的每一条命令的前面。在Bash Shell中，缺省的$PS4的值是"+"号。
