...
可以，但没法达到预期效果，因为这时是基类对象，不是派生类对象
...
1. 内核引导：打开电源后，BIOS开机自检，/boot目录下的内核文件
2. 运行init进程：init进程是所有进程的老祖宗，其配置文件是/etc/inittab
3. 设置运行等级：根据/etc/inittab文件设置运行等级，比如有无网络
4. 系统初始化：启动第一个用户层文件/etc/rc.d/rc.sysinit，然后激活交换分区、检查磁盘、加载硬件模块等
5. 建立终端：系统初始化后返回init，这时守护进程也启动了，init接下来打开终端以供用户登录
