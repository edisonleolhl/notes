...
父进程使用fork拷贝出来一个父进程的副本，只拷贝了父进程的页表，两个进程都读同一块内存，当有进程写的时候使用写时拷贝机制（见下点）分配内存
...
5. Hash法：如果这些数据中有很多重复的数据，可以先通过hash法，把重复的数去掉，这样可以大大减少运算量

6. 如果允许分布式，可以把数据分发给多台机器，每台机器并行计算各自的TopK，最后再汇总，得到最终的TopK，比如MapReduce分布式计算框架
