...
重定义/隐藏（redefining)：子类重新定义父类中同名的**非虚函数 (参数列表可以不同)**或父类中**同名的虚函数（但参数列表不同）**，此时基类的函数被隐藏，重定义比重写的范围更广，因为参数不加限定
...
Linux中，无论进程线程，在内核中都叫任务(task)，统一由task_struct管理，底层用到了链表（为了方便进程的插入和删除），链表的每个节点就是一个task_struct，其中包含了信号处理相关的操作，进程调度牵涉到优先级与调度策略等等
...
2. 读写锁：rwlock，可以允许多个线程同时获得读操作。但是同一时刻**只有一个线程可以获得写锁**。写锁会阻塞其它读写锁。
   ...
   - 读优先：写者、读者互斥访问文件资源，多个读者可以同时访问文件资源，只允许一个写者访问文件资源。
...
可以开启**epoll oneshot**（确保一个socket只被一个进程/线程使用）
...
D类：多播地址，网络号前四位固定为1110，所以用点分十进制的开头是224~239之间
...
3. **header压缩**：因为HTTP协议无状态，header的很多字段重复发送了，HTTP2.0使用HPACK算法来减少传输的header的大小，双方共同维护一个首部表，增量更新即可
...
- 在随时可能出现异常的分布式系统中，可以快速且正确地对集群内某个数据的值达成一致
- 基于二阶段提交协议与过半理论
- 第一个被证明的共识算法
