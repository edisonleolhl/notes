...
以下代码是自己测试的，整体思路与题38相差不大，最后返回的res是满足条件的所有排列
...
第一次尝试：直接递归遍历左右子树，递归函数的参数一个是当前深度cur，一个是已记录的最大深度depth，注意depth要用引用，这样可以修改实参的值
...
如果不是二叉搜索树，但有指向父节点的指针，那么问题可以转化为求两个链表的第一个公共节点，这在题52中有做过
